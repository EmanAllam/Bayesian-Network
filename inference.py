...
def constructBayesNet(gameState: hunters.GameState):
    ...
    """
    """Add variables and edges based on the diagram.                                                                                DONE
     Pacman and the two ghosts can be anywhere in the grid (we ignore walls for this). Add all possible position tuples for these.  DONE
    ...
    #Ghost1 -> Observation1, Pacman -> Observation1, Ghost0 -> Observation0, Pacman -> Observation0
    ...
    #manhatten distance is x1-x2 + y1-y2
    observe = []
    for i in range(X_RANGE + Y_RANGE + MAX_NOISE - 1):
        observe.append(i)
        
    variableDomainsDict[OBS0] = observe
    variableDomainsDict[OBS1] = observe
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        performing joining over and eliminating that variable,
        until the only the query and evidence variables remain.
        ...
        Look at the inferenceByEnumeration function in inference.py 
        for an example on how to use the desired functions. (Reminder: Inference by enumeration first joins
        over all the variables and then eliminates all the hidden variables. 
        In contrast, 
        ...
        You will need to take care of the special case where a factor you have joined 
        only has one unconditioned variable (the docstring specifies what to do in greater detail).
        ...
        for var in eliminationOrder:
            factors,output= joinFactorsByVariable(factors, var)
            
            unconditioned=output.unconditionedVariables()
            # If a factor that you are about to eliminate a variable from has only one unconditioned variable, you should not eliminate it   and instead just discard the factor.
            if len(unconditioned) != 1:
                
                # Join factors containing the variable
                #joinedFactor = joinFactorsByVariable(factorsWithVar, var)
                #factors = [factor for factor in factors if factor not in factorsWithVar]
                #factors.append(joinedFactor)

                # Eliminate the variable
               factorss = eliminate(output, var)
               factors.append(factorss)
                
        # Combine remaining factors by joining
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        if len(self) != 0 or sum(self.values()) != 0 :
             total_value = self.total()
             for i in self.keys():
                 if total_value!=0:
                    self[i] = self[i] / total_value
                 else:
                     return
        else:
            return
    ...
    def sample(self):
        ...
        rand_num = random.random() * total_value
        current_sum = 0
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        #observation is noisyDistance
        # when we capture a ghost and send it to the jail location and  our distance sensor deterministically returns None
        #observation = None if and only if ghost is in jail
        #consequences:the observation is None with probability 1, and everything else with probability 0
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in keys:
          if pos in all_positions:
                prob = self.getObservationProb(observation, pac_position, pos, jail_position)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for old_position in self.allPositions:
            transition_model = self.getPositionDistribution(gameState, old_position)
            for new_position, probability in transition_model.items():
               new_belief[new_position] += self.beliefs[old_position] * probability
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        if weights.total() == 0:
            self.initializeUniformly(gameState)
             ...
             newParticles = []
             discreteDist = DiscreteDistribution(weights)
             for _ in range(self.numParticles):
                 newParticles.append(discreteDist.sample())
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(len(current_particles)):
            newPosDist = self.getPositionDistribution(gameState, current_particles[i])
            updated_distribution[current_particles[i]] = newPosDist
            current_particles[i] = newPosDist.sample()
